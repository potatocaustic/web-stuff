...
// **MODIFIED**: Function to load data for a specific category from Supabase
...
// **MODIFIED**: Function to compare dates (YYYY-MM-DD) and sort from newest to oldest
function sortByDateDescending(a, b) {
    // Use 'poll_date' from Supabase
...
function selectPlayer(playerName) {
  ...
  // **MODIFIED**: Filter data using 'player_name'
...
// **MODIFIED**: Display player data using new column names
function displayPlayerData(playerData, limitToFive) {
  ...
  if (playerData.length > 0) {
    ...
    dataToDisplay.forEach(entry => {
      ...
      // Format the date from 'poll_date'
      const formattedDate = formatDate(entry.poll_date);

      // Use 'target_value', 'player_rank', and 'hit' columns
      row.innerHTML = `
        <td>${formattedDate}</td>
...
// **MODIFIED**: Update player stats summary using new column names
function updatePlayerStats(playerData) {
  ...
  playerData.forEach(entry => {
    ...
    stats.total += 1;
    // 'hit' is a boolean (true/false), so convert to number (1/0)
  ...
  periods.forEach(period => {
    if(period.value > 0) { // Only show if data exists
        const row = document.createElement('tr');
