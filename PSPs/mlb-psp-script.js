// Global variables
let currentCategory = 'TB'; // Default MLB category
let allData = {
  TB: [],
  RBI: [],
  Ks: []
...
const elements = {
  ...
  ksButton: document.getElementById('ksButton'), // Correct ID is 'ksButton'
...
async function init() {
  ...
  elements.ksButton.addEventListener('click', () => switchCategory('Ks')); // Correct event listener
...
async function loadData(category) {
  ...
  try {
    const { data, error } = await supabaseClient
      .from('mlb_psp_data') // Fetch from MLB table
      .select('*')
...
async function switchCategory(category) {
  ...
  elements.ksButton.classList.toggle('active', category === 'Ks');
...
function getCategoryFullName(category) {
    switch (category) {
        ...
        case 'Ks': return 'Pitching Strikeouts';
        default: return category;
    }
}

// The rest of the functions (handlePlayerSearch, selectPlayer, displayPlayerData, etc.)
// are identical to the previous scripts. They are included below for completeness.
