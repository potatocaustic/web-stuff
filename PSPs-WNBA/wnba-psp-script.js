// Global variables
let currentCategory = 'PTS'; // Default WNBA category
let allData = {
  PTS: [],
  REB: [],
  AST: [],
...
async function init() {
  ...
  elements.threePMButton.addEventListener('click', () => switchCategory("'3PM"));
...
async function loadData(category) {
  ...
  try {
    const { data, error } = await supabaseClient
      .from('wnba_psp_data') // Fetch from WNBA table
      .select('*')
...
async function switchCategory(category) {
  ...
  elements.threePMButton.classList.toggle('active', category === "'3PM");
...
// are identical to the previous scripts and can be copied from them. For completeness,
